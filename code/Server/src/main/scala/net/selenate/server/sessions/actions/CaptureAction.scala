...
import com.instantor.amazon.client.data.uri.S3URIFactory
...
class CaptureAction(val d: FirefoxDriver, s3Client: IS3Client)(implicit context: ActionContext)
  ...
  private def getWindowList(description: String, takeScreenshot: Boolean, screenshotWindowIndex: Int) = {
    ...
    val res: SeWindows = {
      val list = if (context.useFrames)
        seqToRealJava(d.getWindowHandles.toList map { wh => getWindow(wh, takeScreenshot)})
      else
